...
public partial class World
{
    ...
    private HudWindow _pusPanel = null!;
    ...
    private int _pusSelectedCategory = 1;
    ...
    private void ShoppingMallDispose()
    {
        ...
        Net.I.ShoppingMallGiftResultEvent -= OnShoppingMallGiftResult;
        Net.I.ShoppingMallSendResultEvent -= OnShoppingMallSendResult;
        Net.I.ShoppingMallDeleteEvent -= OnShoppingMallDelete;        Net.I.ShoppingMallCatalogEvent -= OnShoppingMallCatalog;
        Net.I.ShoppingMallCategoriesEvent -= OnShoppingMallCategories;
        Net.I.ShoppingMallBalanceEvent -= OnShoppingMallBalance;
        Net.I.ShoppingMallBuyResultEvent -= OnShoppingMallBuyResult;
    }
    ...
    private void BuildPusPanel()
    {
        _pusLayer = new CanvasLayer { Layer = 73 };
        AddChild(_pusLayer);

        _pusPanel = new HudWindow("pus", "Power-Up Store", new Vector2(440, 260)) { Visible = false };
        _pusPanel.Closed += ClosePowerUpStore;
        _pusLayer.AddChild(_pusPanel);

        var root = _pusPanel.Body;
        root.AddThemeConstantOverride("separation", 8);
    ...
    public void OpenPowerUpStore()
    {
        if (_pusShown) return;
        _pusShown = true;
        _pusPanel.Visible = true;
        Net.I.SendShoppingMallOpen();
        RefreshPusView();
        UpdatePusWallet();
    }

    public void ClosePowerUpStore()
    {
        if (!_pusShown) return;
        _pusShown = false;
        _pusPanel.Visible = false;
    }

    public void TogglePowerUpStore()
    {
        if (_pusShown) ClosePowerUpStore();
        else OpenPowerUpStore();
    }
    ...
    private void OnShoppingMallBuyResult(bool ok, int remainingCash)
    {
        if (ok)
        {
            Sheet.SetKnightCash(remainingCash);
            UpdateStatusHud();
            UpdatePusWallet();
            _pusBasketStatus.Text = "Purchase complete.";
            _pusBasketStatus.AddThemeColorOverride("font_color", UiTheme.Good);
            Chat.Info($"Power-Up Store purchase complete. Remaining KC: {remainingCash:n0}.");
        }
        else
        {
            _pusBasketStatus.Text = "Purchase failed (insufficient KC or inventory full).";
            _pusBasketStatus.AddThemeColorOverride("font_color", UiTheme.Bad);
            Chat.Info("Power-Up Store purchase failed.");
        }
    }
}